...
import {
  closeMedicalChatThread,
  getMedicalChatThreadById,
  sendMessageToThread,
...
const PetMedicalChatThreadPage = () => {
  ...
  const loadThread = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await getMedicalChatThreadById(threadId);
      setThread(response);
    } catch (err) {
      setError(getErrorMessage(err, "Failed to load medical chat thread."));
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const previews = files.map((file) => {
      ...
      return { name: file.name, sizeKb: Math.round(file.size / 1024), url };
    });
  ...
  const handleSendMessage = async (event) => {
    ...
    try {
      setSubmitting(true);
      await sendMessageToThread(threadId, { message: message.trim(), emergency });
      setMessage("");
      setEmergency(false);
      await loadThread();
    } catch (err) {
  ...
  const handleUploadImages = async (event) => {
    event.preventDefault();
    if (!files.length) { setUploadError("Please select at least one image."); return; }

    try {
      setUploading(true);
      setUploadError("");
      await uploadImagesToThread(threadId, files, uploadMessage, uploadEmergency);
      setFiles([]);
      setUploadMessage("");
  ...
  const handleCloseThread = async () => {
    const confirmClose = window.confirm(
      "Mark this medical thread as resolved and close it? New replies and uploads will be disabled."
    );
    ...
    try {
      setClosingChat(true);
      setError("");
      const updated = await closeMedicalChatThread(threadId);
      setThread(updated);
    } catch (err) {
      setError(getErrorMessage(err, "Failed to close thread."));
    } finally {
  ...
  if (loading) {
    return <div className="container mt-4">Loading medical chat thread...</div>;
  }
  ...
  return (
    <div className="container mt-4 mb-5">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
          <h3 className="mb-0">
            {thread?.petContext?.petName
              ? `${thread.petContext.petName} — `
              : ""}
            {thread?.chatStatus === "CLOSED" ? "Closed Thread" : "Medical Thread"}
          </h3>
          <small className="text-muted">
            Thread #{thread?.threadId} &middot; Pet ID: {thread?.petId}
          </small>
        </div>
        <div className="d-flex gap-2 align-items-center flex-wrap">
          {thread?.chatStatus === "CLOSED" ? (
            <span className="badge text-bg-secondary">Closed / History</span>
          ) : (
            <span className="badge text-bg-success">Active</span>
          )}
          {thread?.canClose && thread?.chatStatus !== "CLOSED" && (
            <button
              type="button"
            ...
            >
              {closingChat ? "Closing..." : "Close Thread"}
            </button>
          )}
          <Link
            className="btn btn-outline-secondary btn-sm"
            to={`/pet-medical-chat/${thread?.petId}`}
          >
            All Threads
          </Link>
      ...
      {thread?.chatStatus === "CLOSED" && (
        <div className="alert alert-secondary">
          This thread was closed{thread?.closedAt ? ` on ${formatDateTime(thread.closedAt)}` : ""}
          {thread?.closedByName ? ` by ${thread.closedByName}` : ""}. It is now read-only history.
        </div>
      )}
      ...
      <div className="card shadow-sm mb-3">
        ...
        <div className="card-body" style={{ maxHeight: "450px", overflowY: "auto" }}>
          {orderedMessages.length === 0 ? (
            <div className="text-muted">No messages yet. Send the first message below.</div>
          ) : (
      ...
      {/* Send Message */}
      <div className="card shadow-sm mb-3">
        <div className="card-header bg-light">Send Message</div>
        <div className="card-body">
          {!thread?.canReply && (
            <div className="alert alert-light border small">
              This thread is closed. Chat is available in history for reference only.
            </div>
          )}
          <form onSubmit={handleSendMessage}>
            <div className="mb-2">
              <textarea
                ...
                rows="3"
                placeholder="Type your message / advice"
                value={message}
            ...
            <button
              ...
              type="submit"
              disabled={submitting || !message.trim() || !thread?.canReply}
            >
      ...
      {canUploadImages && (
        <div className="card shadow-sm">
          ...
          <div className="card-body">
            {!thread?.canReply && (
              <div className="alert alert-light border small">
                This thread is closed. Image uploads are disabled, but history remains visible.
              </div>
            )}
            <form onSubmit={handleUploadImages}>
              ...
              <button
                ...
                type="submit"
                disabled={uploading || files.length === 0 || !thread?.canReply}
              >
...
export default PetMedicalChatThreadPage;
